...
const FromView: React.FC<FromViewProps> = ({ item }) => {
  ...
  const { value, field, model, read, submit, reset } = useForm(EmployeeDtoModel, {
  ...
  useEffect(() => {
    if (value.id)
      formImage.value = `v1/content/image?imagePath=${btoa(`/org/user/employee/${value.id}/temp/200/${value.id}.png`)}`;
  }, [value.id]);
                ...
                <FromColumn header="Upload">
                  <div className="text-blue-500 inline-flex items-center gap-2 border-b p-2">
                    ...
                    <Upload
                      ...
                      headers={`{"path": "/org/user/employee/${value.id || ''}", "filename": "${value.id || ''}.png" }`}
                      ...
                      onUploadSuccess={(e: UploadSuccessEvent) => {
                        ...
                        formImage.value = `v1/content/image?imagePath=${btoa(`/org/user/employee/${value.id}/temp/200/${value.id}.png`)}&mdt=${new Date().getTime().toFixed(0)}`;
                      }}
                    />
                  </div>
                </FromColumn>
