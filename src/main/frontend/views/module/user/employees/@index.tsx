  ...
  const ItemRender = ({ item }: { item: EmployeeDto }) => {
    ...
    return (
      <div className="text-blue-500 inline-flex items-center gap-2">
        <img
          src={`v1/content/image?imagePath=${btoa(`/org/user/employee/${id}/temp/100/${id}.png`)}`}
          onError={(e: any) => {
      ...
      >
        <header className="w-full">
          <div className="flex justify-between gap-4 px-4 py-2">
            <img
              src={`v1/content/image?imagePath=${btoa(`/org/user/employee/${item?.id}/temp/200/${item?.id}.png`)}`}
              onError={(e: any) => {
                e.target.src = `images/profile/default_profile.png`;
              }}
