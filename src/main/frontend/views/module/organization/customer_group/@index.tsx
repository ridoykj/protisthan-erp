...
import CustomerGroupDto from 'Frontend/generated/com/itbd/protisthan/db/dto/CustomerGroupDto';

import CustomerGroupDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/CustomerGroupDtoModel';

import { CustomerGroupDtoCrudService } from 'Frontend/generated/endpoints';
...
const CustomerGroupsView = () => {
  console.count('UserView');
  const navigate = useNavigate();
  const location = useLocation();

  const selectedItems = useSignal<CustomerGroupDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear, value, read } = useForm(CustomerGroupDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await CustomerGroupDtoCrudService.save(person).then(() => {
        ...
        <FromLayoutRC>
          <FromRow>
            <FromColumn>
              ...
              />
              <TextField label="Default Price List" {...field(model.defaultPriceList)} />
              <TextField label="Payment Terms" {...field(model.paymentTerms)} />
            </FromColumn>
          </FromRow>
        </FromLayoutRC>
      </DialogModalRC>
  ...
  const ItemRender = ({ item }: { item: CustomerGroupDto }) => {
    const { id, name } = item;
    return (
      <div className="text-blue-500 inline-flex items-center gap-2 ">
        <FaPeopleArrows className="text-gray-600 size-8 ring-2 ring-gray-300 p-1 rounded-full" />
        {name}
      </div>
    );
  };
      ...
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ...
            visibleColumns={['name', 'isGroup', 'defaultPriceList', 'paymentTerms']}
            columnOptions={{
              name: {
                header: 'Name',
                renderer: ItemRender,
  ...
  const ItemField = ({ item }: { item: CustomerGroupDto }) => {
    return item ? (
      <TabGroup>
          ...
          <TabPanel>
            <div className="flex flex-col gap-2 divide-y flex-grow ">
              <div className="flex flex-col py-4">
                ...
                <DetailField title="Default Price List" details={item.defaultPriceList ?? ''} icon={<FaPiggyBank />} />
                <DetailField title="Payment Terms" details={item.paymentTerms ?? ''} icon={<FaArrowTrendUp />} />
              </div>
            </div>
          </TabPanel>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<CustomerGroupDto[]> }) => {
...
export default CustomerGroupsView;
