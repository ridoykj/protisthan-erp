...
type FromViewProps = {
  item: CustomerGroupDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset, value } = useForm(CustomerGroupDtoModel, {
    onSubmit: async (values) => {
      await CustomerGroupDtoCrudService.save(values);
    },
  });
...
const CustomerGroupView = () => {
  ...
  const editItem = useSignal<CustomerGroupDto | undefined>(undefined);

  useEffect(() => {
    CustomerGroupDtoCrudService.list(pagination, filterGenerator('and', 'id', customerGroupId)).then((result) => {
