...
type FromViewProps = {
  item: SupplierGroupDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const supplierGroupDataProvider = useMemo(() => SupplierGroupDataProvider, []);
  const { field, model, read, submit, reset, value } = useForm(SupplierGroupDtoModel, {
    onSubmit: async (values) => {
      await SupplierGroupDtoCrudService.save(values);
    },
  });
...
const SupplierGroupView = () => {
  const { supplierGroupId } = useParams();
  console.count('UserView');
  const dialogOpened = useSignal(false);
  const editItem = useSignal<SupplierGroupDto | undefined>(undefined);

  useEffect(() => {
    SupplierGroupDtoCrudService.list(pagination, filterGenerator('and', 'id', supplierGroupId)).then((result) => {
