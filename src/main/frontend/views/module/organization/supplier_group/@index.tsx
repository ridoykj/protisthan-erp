...
import SupplierGroupDto from 'Frontend/generated/com/itbd/protisthan/db/dto/SupplierGroupDto';

import SupplierGroupDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/SupplierGroupDtoModel';

import { SupplierGroupDtoCrudService } from 'Frontend/generated/endpoints';
import { SupplierGroupDataProvider } from 'Frontend/utils/combobox/ComboboxDataProvider';
...
const SupplierGroupsView = () => {
  console.count('UserView');
  const navigate = useNavigate();
  const location = useLocation();

  const selectedItems = useSignal<SupplierGroupDto[]>([]);
  ...
  const DialogView = () => {
    ...
    const { field, model, submit, clear, value, read } = useForm(SupplierGroupDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await SupplierGroupDtoCrudService.save(person).then(() => {
        ...
        <FromLayoutRC>
          <FromRow>
            <FromColumn>
              ...
              <ComboBox
                label="Parent Group"
                {...field(model.parentSupplierGroup)}
  ...
  const ItemRender = ({ item }: { item: SupplierGroupDto }) => {
    const { id, name } = item;
    return (
      <div className="text-blue-500 inline-flex items-center gap-2 ">
        <FaCartFlatbed className="text-gray-600 size-8 ring-2 ring-gray-300 p-1 rounded-full" />
        {name}
      </div>
    );
  };
      ...
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ...
            visibleColumns={['name', 'group', 'parentSupplierGroup.name']}
            columnOptions={{
              name: {
                header: 'Name',
                renderer: ItemRender,
              },
              'parentSupplierGroup.name': {
                header: 'Parent Group',
              },
            }}
  ...
  const ItemField = ({ item }: { item: SupplierGroupDto }) => {
    return item ? (
      <TabGroup>
          ...
          <TabPanel>
            <div className="flex flex-col gap-2 divide-y flex-grow ">
              <div className="flex flex-col py-4">
                ...
                <DetailField
                  title="Parent Group"
                  details={item.parentSupplierGroup?.name ?? ''}
                  icon={<FaRegCircleUser />}
                />
                <DetailField title="Group" details={item.group ? 'Yes' : 'No' ?? ''} icon={<FaEyeLowVision />} />
              </div>
            </div>
          </TabPanel>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<SupplierGroupDto[]> }) => {
...
export default SupplierGroupsView;
