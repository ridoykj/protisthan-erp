...
type FromViewProps = {
  item: DesignationDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset } = useForm(DesignationDtoModel, {
    onSubmit: async (values) => {
      await DesignationDtoCrudService.save(values);
    },
  });
...
const DesignationView = () => {
  ...
  const editItem = useSignal<DesignationDto | undefined>(undefined);

  useEffect(() => {
    DesignationDtoCrudService.list(pagination, filterGenerator('and', 'id', designationId)).then((result) => {
