...
import DesignationDto from 'Frontend/generated/com/itbd/protisthan/db/dto/DesignationDto';

import DesignationDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/DesignationDtoModel';

import { DesignationDtoCrudService } from 'Frontend/generated/endpoints';
...
const DesignationsView = () => {
  ...
  const selectedItems = useSignal<DesignationDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear } = useForm(DesignationDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await DesignationDtoCrudService.save(person).then(() => {
  ...
  const ItemRender = ({ item }: { item: DesignationDto }) => {
    const { id, name } = item;
    return (
      <div className="text-blue-500 inline-flex items-center gap-2">
        <FaAddressCard className="text-gray-600 size-8 ring-2 ring-gray-300 p-1 rounded-full" />
        {name}
      </div>
    );
  };
  ...
  const ItemField = ({ item }: { item: DesignationDto }) => {
    return item ? (
      <TabGroup>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<DesignationDto[]> }) => {
