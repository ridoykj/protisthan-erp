...
type FromViewProps = {
  item: ModeOfPaymentDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset, value } = useForm(ModeOfPaymentDtoModel, {
    onSubmit: async (values) => {
      await ModeOfPaymentDtoCrudService.save(values);
    },
  });
            ...
            <FromLayoutRC>
              <FromRow header="Details">
                ...
                <FromColumn>
                  ...
                  {/* <TextField label="Mode Of Payment" {...field(model.modeOfPayment)} /> */}
...
const ModeOfPaymentView = () => {
  const { modeOfPaymentId } = useParams();
  const dialogOpened = useSignal(false);
  const editItem = useSignal<ModeOfPaymentDto | undefined>(undefined);

  useEffect(() => {
    ModeOfPaymentDtoCrudService.list(pagination, filterGenerator('and', 'id', modeOfPaymentId)).then((result) => {
