...
import ModeOfPaymentDto from 'Frontend/generated/com/itbd/protisthan/db/dto/ModeOfPaymentDto';

import ModeOfPaymentDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/ModeOfPaymentDtoModel';

import { ModeOfPaymentDtoCrudService } from 'Frontend/generated/endpoints';
...
const ModeOfPaymentsView = () => {
  console.count('UserView');
  const navigate = useNavigate();
  const location = useLocation();

  const selectedItems = useSignal<ModeOfPaymentDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear, value, read } = useForm(ModeOfPaymentDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await ModeOfPaymentDtoCrudService.save(person).then(() => {
        ...
        <FromLayoutRC>
          <FromRow>
            <FromColumn>
              ...
              <TextField label="Type" {...field(model.type)} />
              <SwitchRC
                label="Enabled"
  ...
  const ItemRender = ({ item }: { item: ModeOfPaymentDto }) => {
    const { id, name } = item;
    return (
      <div className="text-blue-500 inline-flex items-center gap-2">
        <FaCreditCard className="text-gray-600 size-8 ring-2 ring-gray-300 p-1 rounded-full" />
        {name}
      </div>
    );
  };
      ...
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ...
            visibleColumns={['name', 'isEnabled', 'type']}
            columnOptions={{
              name: {
                header: 'Name',
                renderer: ItemRender,
  ...
  const ItemField = ({ item }: { item: ModeOfPaymentDto }) => {
    return item ? (
      <TabGroup>
          ...
          <TabPanel>
            <div className="flex flex-col gap-2 divide-y flex-grow ">
              <div className="flex flex-col py-4">
                ...
                <DetailField title="Enabled" details={item.isEnabled ? 'Yes' : 'No' ?? ''} icon={<FaEyeLowVision />} />
              </div>
            </div>
          </TabPanel>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<ModeOfPaymentDto[]> }) => {
...
export default ModeOfPaymentsView;
