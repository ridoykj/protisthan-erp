...
import { ComboBox, TextField, Upload } from '@vaadin/react-components';
...
const FromView: React.FC<FromViewProps> = ({ item }) => {
  ...
  const { field, model, read, submit, reset } = useForm(SupplierDtoModel, {
    onSubmit: async (values) => {
      await SupplierDtoCrudService.save(values);
    },
  });
            ...
            <FromLayoutRC>
              <FromRow header="Details">
                ...
                <FromColumn>
                  ...
                  <TextField label="Postal Code" {...field(model.postalCode)} />
                ...
                <FromColumn>
                  ...
                  <TextField label="Fax" {...field(model.fax)} />
                  <TextField label="Homepage" {...field(model.homepage)} />
                </FromColumn>
              </FromRow>
            </FromLayoutRC>
          </div>
...
const SupplierView = () => {
  ...
  const editItem = useSignal<SupplierDto | undefined>(undefined);

  useEffect(() => {
    SupplierDtoCrudService.list(pagination, filterGenerator('and', 'id', supplierId)).then((result) => {
