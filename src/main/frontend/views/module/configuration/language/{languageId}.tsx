...
type FromViewProps = {
  item: LanguageDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset, value } = useForm(LanguageDtoModel, {
    onSubmit: async (values) => {
      await LanguageDtoCrudService.save(values);
    },
  });
...
const LanguageView = () => {
  ...
  const editItem = useSignal<LanguageDto | undefined>(undefined);

  useEffect(() => {
    LanguageDtoCrudService.list(pagination, filterGenerator('and', 'id', languageId)).then((result) => {
