...
import LanguageDto from 'Frontend/generated/com/itbd/protisthan/db/dto/LanguageDto';
import LanguageDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/LanguageDtoModel';

import { LanguageDtoCrudService } from 'Frontend/generated/endpoints';
...
const LanguagesView = () => {
  ...
  const selectedItems = useSignal<LanguageDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear } = useForm(LanguageDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await LanguageDtoCrudService.save(person).then(() => {
        ...
        <FromLayoutRC>
          <FromRow>
            <FromColumn>
              ...
              <TextField label="Flag" {...field(model.flag)} />
            </FromColumn>
          </FromRow>
        </FromLayoutRC>
      </DialogModalRC>
  ...
  const ItemRender = ({ item }: { item: LanguageDto }) => {
    const { id, name } = item;
    return (
      <div className="text-blue-500 inline-flex items-center gap-2">
        <FaLanguage className="text-gray-600 size-8 ring-2 ring-gray-300 p-1 rounded-full" />
        {id}
      </div>
    );
  };
      ...
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ...
            visibleColumns={['id', 'name', 'isEnabled', 'dateFormat', 'timeFormat']}
            columnOptions={{
              id: {
                header: 'Code',
                renderer: ItemRender,
  ...
  const ItemField = ({ item }: { item: LanguageDto }) => {
    return item ? (
      <TabGroup>
          ...
          <TabPanel>
            <div className="flex flex-col gap-2 divide-y flex-grow ">
              <div className="flex flex-col py-4">
                ...
                <DetailField title="Flag" details={item.flag ?? ''} icon={<FaFlag />} />
  ...
  const ItemDetails = ({ employee }: { employee: Signal<LanguageDto[]> }) => {
