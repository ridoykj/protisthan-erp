...
type FromViewProps = {
  item: UomConversionFactorDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const uomCategoryDataProvider = useMemo(() => UomCategoryDataProvider, []);
  const { field, model, read, submit, reset, value } = useForm(UomConversionFactorDtoModel, {
    onSubmit: async (values) => {
      await UomConversionFactorDtoCrudService.save(values);
    },
  });
...
const UomConversionFactorView = () => {
  const { uomConversionFactorId } = useParams();
  console.count('UserView');
  const dialogOpened = useSignal(false);
  const editItem = useSignal<UomConversionFactorDto | undefined>(undefined);

  useEffect(() => {
    UomConversionFactorDtoCrudService.list(pagination, filterGenerator('and', 'id', uomConversionFactorId)).then(
      (result) => {
