...
type FromViewProps = {
  item: CountryDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset } = useForm(CountryDtoModel, {
    onSubmit: async (values) => {
      await CountryDtoCrudService.save(values);
    },
  });
...
const CountryView = () => {
  ...
  const editItem = useSignal<CountryDto | undefined>(undefined);

  useEffect(() => {
    CountryDtoCrudService.list(pagination, filterGenerator('and', 'id', countryId)).then((result) => {
