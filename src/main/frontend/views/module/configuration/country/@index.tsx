...
import CountryDto from 'Frontend/generated/com/itbd/protisthan/db/dto/CountryDto';
import CountryDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/CountryDtoModel';

import { CountryDtoCrudService } from 'Frontend/generated/endpoints';
...
const CountrysView = () => {
  ...
  const selectedItems = useSignal<CountryDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear } = useForm(CountryDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await CountryDtoCrudService.save(person).then(() => {
        ...
        <FromLayoutRC>
          <FromRow>
            <FromColumn>
              ...
              <TextField label="Date Format" {...field(model.dateFormat)} />
              <TextField label="Time Format" {...field(model.timeFormat)} />
              <TextField label="Time Zones" {...field(model.timeZones)} />
            </FromColumn>
          </FromRow>
        </FromLayoutRC>
      </DialogModalRC>
  ...
  const ItemRender = ({ item }: { item: CountryDto }) => {
    const { id, name } = item;
    return (
      <div className="text-blue-500 inline-flex items-center gap-2">
        <FaEarthAmericas className="text-gray-600 size-8 ring-2 ring-gray-300 p-1 rounded-full" />
        {id}
      </div>
    );
  };
      ...
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ...
            visibleColumns={['id', 'name', 'dateFormat', 'timeFormat']}
            columnOptions={{
              id: {
                header: 'Code',
                renderer: ItemRender,
  ...
  const ItemField = ({ item }: { item: CountryDto }) => {
    return item ? (
      <TabGroup>
          ...
          <TabPanel>
            <div className="flex flex-col gap-2 divide-y flex-grow ">
              <div className="flex flex-col py-4">
                ...
                <DetailField title="Date Format" details={item.dateFormat ?? ''} icon={<FaCalendarDay />} />
                <DetailField title="Time Format" details={item.timeFormat ?? ''} icon={<FaClock />} />
                <DetailField title="Time Zones" details={item.timeZones ?? ''} icon={<FaEarthAmericas />} />
              </div>
            </div>
          </TabPanel>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<CountryDto[]> }) => {
