...
type FromViewProps = {
  item: UomDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset, value } = useForm(UomDtoModel, {
    onSubmit: async (values) => {
      await UomDtoCrudService.save(values);
    },
  });
...
const UomView = () => {
  ...
  const editItem = useSignal<UomDto | undefined>(undefined);

  useEffect(() => {
    UomDtoCrudService.list(pagination, filterGenerator('and', 'id', uomId)).then((result) => {
