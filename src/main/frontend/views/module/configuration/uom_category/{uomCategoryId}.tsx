...
type FromViewProps = {
  item: UomCategoryDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset } = useForm(UomCategoryDtoModel, {
    onSubmit: async (values) => {
      await UomCategoryDtoCrudService.save(values);
    },
  });
...
const UomCategoryView = () => {
  const { uomCategoryId } = useParams();
  console.count('UserView');
  const dialogOpened = useSignal(false);
  const editItem = useSignal<UomCategoryDto | undefined>(undefined);

  useEffect(() => {
    UomCategoryDtoCrudService.list(pagination, filterGenerator('and', 'id', uomCategoryId)).then((result) => {
