...
import UomCategoryDto from 'Frontend/generated/com/itbd/protisthan/db/dto/UomCategoryDto';
import UomCategoryDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/UomCategoryDtoModel';

import { UomCategoryDtoCrudService } from 'Frontend/generated/endpoints';
...
const UomCategorysView = () => {
  console.count('UserView');
  const navigate = useNavigate();
  const location = useLocation();

  const selectedItems = useSignal<UomCategoryDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear } = useForm(UomCategoryDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await UomCategoryDtoCrudService.save(person).then(() => {
  ...
  const ItemRender = ({ item }: { item: UomCategoryDto }) => {
    const { id } = item;
    return (
  ...
  const ItemField = ({ item }: { item: UomCategoryDto }) => {
    return item ? (
      <TabGroup>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<UomCategoryDto[]> }) => {
...
export default UomCategorysView;
