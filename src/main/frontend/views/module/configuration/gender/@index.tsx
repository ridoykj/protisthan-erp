...
import GenderDto from 'Frontend/generated/com/itbd/protisthan/db/dto/GenderDto';

import GenderDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/GenderDtoModel';

import { GenderDtoCrudService } from 'Frontend/generated/endpoints';
...
const GendersView = () => {
  ...
  const selectedItems = useSignal<GenderDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear } = useForm(GenderDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await GenderDtoCrudService.save(person).then(() => {
  ...
  const ItemRender = ({ item }: { item: GenderDto }) => {
    const { id, name } = item;
    return (
      <div className="text-blue-500 inline-flex items-center gap-2">
        <FaTransgender className="text-gray-600 size-8 ring-2 ring-gray-300 p-1 rounded-full" />
        {name}
      </div>
    );
  };
      ...
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ...
            visibleColumns={['name', 'creation', 'modified']}
            columnOptions={{
              name: {
                header: 'Name',
                renderer: ItemRender,
  ...
  const ItemField = ({ item }: { item: GenderDto }) => {
    return item ? (
      <TabGroup>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<GenderDto[]> }) => {
