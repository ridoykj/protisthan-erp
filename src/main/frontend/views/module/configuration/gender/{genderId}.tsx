...
type FromViewProps = {
  item: GenderDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset } = useForm(GenderDtoModel, {
    onSubmit: async (values) => {
      await GenderDtoCrudService.save(values);
    },
  });
...
const GenderView = () => {
  ...
  const editItem = useSignal<GenderDto | undefined>(undefined);

  useEffect(() => {
    GenderDtoCrudService.list(pagination, filterGenerator('and', 'id', genderId)).then((result) => {
