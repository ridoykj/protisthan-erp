...
import StockEntryTypeDto from 'Frontend/generated/com/itbd/protisthan/db/dto/StockEntryTypeDto';

import StockEntryTypeDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/StockEntryTypeDtoModel';

import { StockEntryTypeDtoCrudService } from 'Frontend/generated/endpoints';
...
const StockEntryTypesView = () => {
  console.count('UserView');
  const navigate = useNavigate();
  const location = useLocation();

  const selectedItems = useSignal<StockEntryTypeDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear, value, read } = useForm(StockEntryTypeDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await StockEntryTypeDtoCrudService.save(person).then(() => {
  ...
  const ItemRender = ({ item }: { item: StockEntryTypeDto }) => {
    const { id } = item;
    return (
      ...
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ...
            visibleColumns={['id', 'isAddToTransit']}
            columnOptions={{
              id: {
                header: 'name',
                renderer: ItemRender,
  ...
  const ItemField = ({ item }: { item: StockEntryTypeDto }) => {
    return item ? (
      <TabGroup>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<StockEntryTypeDto[]> }) => {
...
export default StockEntryTypesView;
