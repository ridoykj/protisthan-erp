...
type FromViewProps = {
  item: StockEntryTypeDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset, value } = useForm(StockEntryTypeDtoModel, {
    onSubmit: async (values) => {
      await StockEntryTypeDtoCrudService.save(values);
    },
  });
...
const StockEntryTypeView = () => {
  const { stockEntryId } = useParams();
  const dialogOpened = useSignal(false);
  const editItem = useSignal<StockEntryTypeDto | undefined>(undefined);

  useEffect(() => {
    StockEntryTypeDtoCrudService.list(pagination, filterGenerator('and', 'id', stockEntryId)).then((result) => {
