...
import { DatePicker, TextField, Upload } from '@vaadin/react-components';
...
type FromViewProps = {
  item: OrderDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset } = useForm(OrderDtoModel, {
    onSubmit: async (values) => {
      await OrderDtoCrudService.save(values);
    },
  });
            ...
            <FromLayoutRC>
              <FromRow header="Details">
                ...
                <FromColumn>
                  ...
                  {/* <TextField label="Ship Name" {...field(model.shipName)} /> */}
                </FromColumn>
              </FromRow>
            </FromLayoutRC>
          </div>
...
const OrderView = () => {
  ...
  const editItem = useSignal<OrderDto | undefined>(undefined);

  useEffect(() => {
    OrderDtoCrudService.list(pagination, filterGenerator('and', 'id', orderId)).then((result) => {
