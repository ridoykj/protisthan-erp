...
import { DatePicker, TextField } from '@vaadin/react-components';
...
import OrderDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/OrderDtoModel';

import { OrderDtoCrudService } from 'Frontend/generated/endpoints';
import React from 'react';
import { FaCalendarDay, FaLaptopCode, FaTransgenderAlt, FaUserPlus } from 'react-icons/fa';
import { FaAddressBook, FaArrowsRotate, FaCartPlus, FaPenToSquare, FaTrash } from 'react-icons/fa6';
...
const OrdersView = () => {
  ...
  const selectedItems = useSignal<OrderDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear } = useForm(OrderDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await OrderDtoCrudService.save(person).then(() => {
    ...
    return (
      <DialogModalRC headerTitle="Add Item" opened={dialogOpened} onSave={submit}>
        <FromLayoutRC>
          <FromRow>
            <FromColumn>
              <DatePicker label="Order Date" {...field(model.orderDate)} />
              <DatePicker label="Required Date" {...field(model.requiredDate)} />
              <DatePicker label="Shipped Date" {...field(model.shippedDate)} />
              <TextField label="Freight" {...field(model.freight)} />
            </FromColumn>
          </FromRow>
        </FromLayoutRC>
      </DialogModalRC>
  ...
  const ItemRender = ({ item }: { item: OrderDto }) => {
    const { id } = item;
    return (
      <div className="text-blue-500 inline-flex items-center gap-2">
        <FaCartPlus className="text-gray-600 size-8 ring-2 ring-gray-300 p-1 rounded-full" />
        {id}
      </div>
    );
  };
  ...
  const ItemField = ({ item }: { item: OrderDto }) => {
    return item ? (
      <TabGroup>
          ...
          <TabPanel>
            <div className="flex flex-col gap-2 divide-y flex-grow ">
              <div className="flex flex-col py-4">
                ...
                <DetailField title="Name" details={item.orderDate ?? ''} icon={<FaAddressBook />} />
                <DetailField title="Qty" details={item.requiredDate ?? ''} icon={<FaTransgenderAlt />} />
                <DetailField title="Unit Price" details={item.shippedDate ?? ''} icon={<FaCalendarDay />} />
              </div>
            </div>
          </TabPanel>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<OrderDto[]> }) => {
    const item = employee.value[0];
    return (
      <DetailSectionRC
        headerTitle={item?.id?.toString()}
        ...
        <header className="w-full">
          <div className="flex justify-between gap-4 px-4 py-2">
            ...
            <div className="flex flex-col grow items-left">
              <span className="font-semibold text-lg">{`Name: ${item?.id?.toString() ?? ''}`}</span>
  ...
  return (
    <>
      ...
      <SpeedDialRC>
        <SpeedDialNode name="Import" icon={<FaLaptopCode />} />
        <SpeedDialNode name="User Permissions" icon={<FaLaptopCode />} />
        <SpeedDialNode name="Role Permissions Manager" icon={<FaLaptopCode />} />
        <SpeedDialNode
          name="Add Item"
          icon={<FaUserPlus />}
          onClick={() => {
