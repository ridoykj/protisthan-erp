...
const DocTypesView = () => {
  ...
  const selectedItems = useSignal<DocTypeDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear, value, read } = useForm(DocTypeDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await DocTypeDtoCrudService.save(person).then(() => {
  ...
  const ItemRender = ({ item }: { item: DocTypeDto }) => {
    const { id } = item;
    return (
    ...
    return (
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ref={autoGridRef}
            service={DocTypeDtoCrudService}
            model={DocTypeDtoModel}
            className="h-full w-full overflow-auto bg-white/40"
            selectedItems={selectedItems.value}
            visibleColumns={['id', 'isCustom']}
            columnOptions={{
              id: {
                header: 'Name',
                renderer: ItemRender,
  ...
  const ItemField = ({ item }: { item: DocTypeDto }) => {
    return item ? (
      <TabGroup>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<DocTypeDto[]> }) => {
