...
import { RoleDtoCrudService } from 'Frontend/generated/endpoints';
...
const RolesView = () => {
  ...
  const selectedItems = useSignal<RoleDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear, value, read } = useForm(RoleDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await RoleDtoCrudService.save(person).then(() => {
  ...
  const ItemRender = ({ item }: { item: RoleDto }) => {
    const { id } = item;
    return (
      ...
      <section className="flex flex-col w-full h-full items-stretch ">
        ...
        <main className="w-full h-full">
          <AutoGrid
            ...
            visibleColumns={['id', 'isDisabled', 'isBulkActions', 'isDeskAccess', 'isNotifications', 'twoFactorAuth']}
            columnOptions={{
              id: {
                header: 'Name',
                renderer: ItemRender,
  ...
  const ItemField = ({ item }: { item: RoleDto }) => {
    return item ? (
      <TabGroup>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<RoleDto[]> }) => {
