...
import UserTypeDto from 'Frontend/generated/com/itbd/protisthan/db/dto/UserTypeDto';
import UserTypeDtoModel from 'Frontend/generated/com/itbd/protisthan/db/dto/UserTypeDtoModel';

import { UserTypeDtoCrudService } from 'Frontend/generated/endpoints';
...
const UserTypesView = () => {
  ...
  const selectedItems = useSignal<UserTypeDto[]>([]);
  ...
  const DialogView = () => {
    const { field, model, submit, clear, read, value } = useForm(UserTypeDtoModel, {
      onSubmit: async (person) => {
        console.log(person);
        await UserTypeDtoCrudService.save(person).then(() => {
    ...
    return (
      <DialogModalRC headerTitle="Add Category" opened={dialogOpened} onSave={submit}>
        <FromLayoutRC>
          <FromRow>
            <FromColumn>
              <TextField label="Category Name" {...field(model.id)} />
              <SwitchRC
                label="Standard"
  ...
  const ItemRender = ({ item }: { item: UserTypeDto }) => {
    const { id } = item;
    return (
  ...
  const ItemField = ({ item }: { item: UserTypeDto }) => {
    return item ? (
      <TabGroup>
          ...
          <TabPanel>
            <div className="flex flex-col gap-2 divide-y flex-grow ">
              <div className="flex flex-col py-4">
                ...
                <DetailField title="Category Name" details={item.id ?? ''} icon={<FaAddressBook />} />
                <DetailField
                  title="Standard"
                  details={item.isStandard ? 'Yes' : 'No' ?? ''}
                  icon={<FaEyeLowVision />}
                />
              </div>
            </div>
  ...
  const ItemDetails = ({ employee }: { employee: Signal<UserTypeDto[]> }) => {
