...
type FromViewProps = {
  item: UserTypeDto | undefined;
};

const FromView: React.FC<FromViewProps> = ({ item }) => {
  const { field, model, read, submit, reset, value } = useForm(UserTypeDtoModel, {
    onSubmit: async (values) => {
      await UserTypeDtoCrudService.save(values);
    },
  });
...
const UserTypeDtoView = () => {
  const { userTypeId } = useParams();
  console.count('UserView');
  const dialogOpened = useSignal(false);
  const editItem = useSignal<UserTypeDto | undefined>(undefined);

  useEffect(() => {
    UserTypeDtoCrudService.list(pagination, filterGenerator('and', 'id', userTypeId)).then((result) => {
...
export default UserTypeDtoView;
